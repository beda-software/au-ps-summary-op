...
const main = async () => {
  ...
  fastify.decorateRequest('appToken', Buffer.from(`${config.app.baseUrl}:${config.app.secret}`).toString('base64'));
  ...
  fastify.route({
    method: 'POST',
    url: config.app.callbackUrl.startsWith('/') ? config.app.callbackUrl : `/${config.app.callbackUrl}`,
      ...
      json: {
        ...
        endpoint: {
          url: `${config.app.baseUrl}${
            config.app.callbackUrl.startsWith('/') ? config.app.callbackUrl : `/${config.app.callbackUrl}`
          }`,
          type: 'http-rpc',
